...
export default function TripDetail() {
  ...
  const tripGuests = guests.filter((g) => g.tripId === trip.id);
  const confirmedGuests = tripGuests.filter((g) => g.attending === 'full' || g.attending === 'partial');
  const unsureGuests = tripGuests.filter((g) => g.attending === 'unsure');
  const dietaryGuests = tripGuests.filter((g) => g.dietary);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {trip.budget > 0 ? (
            <>
              ...
              <View style={styles.progressTrack}>
                <View
                  style={[styles.progressFill, { width: `${budgetPercent}%`, backgroundColor: budgetLeft >= 0 ? '#2E9E6B' : '#DC2626' }]}
                />
              </View>
        ...
        <Pressable
          ...
          onPress={() => router.push({ pathname: '/trip/[id]/guests', params: { id } })}
        >
          <Text style={styles.guestCardTitle}>Guest List</Text>
          <Text style={styles.guestCardSummary}>
            {confirmedGuests.length} confirmed
            {unsureGuests.length > 0 ? ` · ${unsureGuests.length} unsure` : ''}
            {dietaryGuests.length > 0 ? ` · ${dietaryGuests.length} dietary` : ''}
          </Text>
          <Text style={styles.guestCardLink}>View full guest list →</Text>
        </Pressable>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>Activities ({tripActivities.length})</Text>
          <PrimaryButton
            compact
            label="+ Add"
            onPress={() => router.push({ pathname: '/trip/[id]/add-activity', params: { id } })}
          />
        </View>
        ...
        {tripActivities.map((activity) => (
          <View key={activity.id} style={styles.activityCard}>
            <View style={styles.activityHeader}>
              <View style={[styles.categoryDot, { backgroundColor: getCategoryColour(activity.categoryId) }]} />
            ...
            <View style={styles.activityMeta}>
              ...
              {activity.duration > 0 ? <Text style={styles.metaText}>{activity.duration} min</Text> : null}
            ...
            <View style={styles.activityCostRow}>
              ...
              {trip.guestCount > 0 ? (
                <Text style={styles.activityPerPerson}>(€{Math.round(activity.cost / trip.guestCount)}/person)</Text>
              ) : null}
            </View>
            ...
            <View style={styles.activityButtons}>
              <Pressable onPress={() => router.push({ pathname: '/activity/[id]/edit', params: { id: activity.id.toString() } })}>
...
const styles = StyleSheet.create({
  ...
  notes: { color: '#6B7280', fontSize: 14, marginBottom: 12, fontStyle: 'italic' },
  budgetCard: { backgroundColor: '#FFFFFF', borderColor: '#F0C6D4', borderRadius: 12, borderWidth: 1, marginBottom: 12, padding: 14 },
  budgetTitle: { color: '#1F1126', fontSize: 16, fontWeight: '700', marginBottom: 8 },
  budgetRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 },
  budgetLabel: { color: '#6B7280', fontSize: 14 },
  budgetValue: { color: '#1F1126', fontSize: 14, fontWeight: '600' },
  ...
  budgetRemaining: { color: '#2E9E6B', fontSize: 13, fontWeight: '600', marginTop: 4 },
  overBudget: { color: '#DC2626' },
  guestCard: { backgroundColor: '#FFFFFF', borderColor: '#F0C6D4', borderRadius: 12, borderWidth: 1, marginBottom: 16, padding: 14 },
  guestCardTitle: { color: '#1F1126', fontSize: 16, fontWeight: '700', marginBottom: 4 },
  guestCardSummary: { color: '#6B7280', fontSize: 14 },
  guestCardLink: { color: '#D4537E', fontSize: 13, fontWeight: '600', marginTop: 8 },
  ...
  sectionTitle: { color: '#1F1126', fontSize: 20, fontWeight: '700' },
  activityCard: { backgroundColor: '#FFFFFF', borderColor: '#F0C6D4', borderRadius: 12, borderWidth: 1, marginBottom: 10, padding: 12 },
  activityHeader: { alignItems: 'center', flexDirection: 'row' },
  categoryDot: { borderRadius: 6, height: 12, marginRight: 8, width: 12 },
  activityName: { color: '#1F1126', fontSize: 16, fontWeight: '600', flex: 1 },
  activityMeta: { flexDirection: 'row', gap: 12, marginTop: 6 },
  metaText: { color: '#6B7280', fontSize: 13 },
  activityCostRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 6 },
  activityCost: { color: '#1F1126', fontSize: 15, fontWeight: '700' },
  activityPerPerson: { color: '#9CA3AF', fontSize: 12 },
  activityNotes: { color: '#9CA3AF', fontSize: 13, fontStyle: 'italic', marginTop: 6 },
  activityButtons: { flexDirection: 'row', gap: 16, marginTop: 8 },
  editLink: { color: '#D4537E', fontSize: 13, fontWeight: '600' },
  deleteLink: { color: '#DC2626', fontSize: 13, fontWeight: '600' },
  bottomButtons: { marginTop: 24, paddingBottom: 30 },
  spacer: { height: 10 },
  emptyText: { color: '#9CA3AF', fontSize: 14, marginBottom: 16, textAlign: 'center' },
});
